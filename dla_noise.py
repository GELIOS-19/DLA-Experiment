    ...
    for i in range(self.size):
      ...
      for j in range(self.size):
        normalized_weights[i].append(
          self.grid[j][i].representative_weight / maximum_weight
        )
  ...
  def pad(matrix, pad_to_size):
    ...
    for i in range(rows):
      for j in range(column):
        padded_array[start_row + i][start_column + j] = matrix[i][
          j
        ]
  ...
  for subject, downstream_count in downstream_counts:
    ...
    if falloff_mode == "smooth":
      ...
      new_image[x, y].weight = (
        ...
        * shifted_exponential(
          downstream_count,
          (exponential_detail_falloff / maximum_downstream_count)
          + 1,
    ...
    while image.density < step_density_threshold:
      ...
      if use_concurrent_walkers:
        ...
        num_concurrent_walkers = max(
          1,
          int(
            (step_density_threshold * total_pixels) - frozen_pixels
          ),
        )
      else:
